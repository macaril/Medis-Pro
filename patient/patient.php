...
$stmt->execute([$useremail]); // PDO: Execute
$userfetch = $stmt->fetch(PDO::FETCH_ASSOC); // PDO: Fetch user details
...
<body>
  <div class="container">
    ...
    // Default Query: My patients Only (Dikonversi ke PDO untuk konsistensi)
    ...
    if ($_POST) {
      ...
      if (isset($_POST["search"])) {
        ...
        $sqlmain = "SELECT * FROM patient WHERE pemail ILIKE ? OR pname ILIKE ? OR pname ILIKE ? OR pname ILIKE ? OR pname ILIKE ?";
        $stmt_search = $database->prepare($sqlmain);
        $search_term = "%$keyword%";
        $stmt_search->execute([$search_term, $search_term, $search_term, $search_term, $search_term]); // 5 times due to original logic
        $result = $stmt_search; // Menggunakan PDOStatement untuk listing
        $selecttype = "Search";
      }
    ...
    // --- Query Utama untuk Listing ---
    // Jika $result belum didefinisikan dari proses search, eksekusi query default
    if (!isset($result)) {
      $result = $database->query($sqlmain);
    }
    
    // --- Query untuk Datalist (Harus dieksekusi di sini) ---
    $datalist_result = $database->query("SELECT * FROM patient;");
