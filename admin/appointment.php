...
<body>
            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <tr>
                    <td colspan="4" style="padding-top:10px;width: 100%;">
                        <p class="heading-main12" style="margin-left: 45px;font-size:18px;color:rgb(49, 49, 49)">All Appointments (<?php echo $list110->rowCount(); ?>)</p>
                    </td>
                ...
                $sqlmain_base = "SELECT appointment.appoid, schedule.scheduleid, schedule.title, doctor.docname, patient.pname, schedule.scheduledate, schedule.scheduletime, appointment.apponum, appointment.appodate
                                 FROM schedule 
                                 INNER JOIN appointment ON schedule.scheduleid::text = appointment.scheduleid::text 
                ...
                if (count($where_clauses) > 0) {
                    $sqlmain = $sqlmain_base . " WHERE " . implode(" AND ", $where_clauses) . " ORDER BY schedule.scheduledate DESC";
                } else {
                    $sqlmain = $sqlmain_base . " ORDER BY schedule.scheduledate DESC";
                }
    ...
    if ($_GET) {
        ...
        if ($action == 'add-session') {
            ...
            $row = $result->fetch(PDO::FETCH_ASSOC); // PERBAIKAN: fetch_assoc() ke fetch(PDO::FETCH_ASSOC)
            $name = $row["docname"];
            $email = $row["docemail"];
            $spe = $row["specialties"];

            $spcil_res = $database->query("select sname from specialties where id='$spe'");
            $spcil_array = $spcil_res->fetch(PDO::FETCH_ASSOC); // PERBAIKAN: fetch_assoc() ke fetch(PDO::FETCH_ASSOC)
