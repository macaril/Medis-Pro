...
<body>
  <div class="container">
    ...
    <?php
    // --- Logika Filtering dan Pencarian ---
    $selecttype = "My";
    $current = "My patients Only";

    // PERBAIKAN KRITIS: Default Query (My Patients Only) - Menggunakan type casting di WHERE
    $sqlmain_base = "SELECT * FROM appointment 
                    INNER JOIN patient ON patient.pid = appointment.pid 
                    INNER JOIN schedule ON schedule.scheduleid = appointment.scheduleid 
                    WHERE schedule.docid = ?::text"; // Cast $userid (int) to TEXT for comparison in WHERE
    $params = [$userid];
    ...
    if ($_POST) {
      ...
      if (isset($_POST["search"])) {
        ...
        // Query hanya mencari di tabel patient
        // PERBAIKAN: Menggunakan ILIKE untuk PostgreSQL dan Prepared Statement
        $sqlmain = "SELECT * FROM patient WHERE pemail ILIKE ? OR pname ILIKE ? OR pnik ILIKE ? OR ptel ILIKE ? ORDER BY pid DESC";
        $stmt_search = $database->prepare($sqlmain);
        $search_term = "%$keyword%";
        
        // Eksekusi dengan parameter
        $stmt_search->execute([$search_term, $search_term, $search_term, $search_term]); 
        $result = $stmt_search;
        $selecttype = "Search Result";
        $current = "Search Result";
        $params = []; // Reset params karena result sudah di-set
        
      } else if (isset($_POST["filter"])) {
        $params = []; // Reset params untuk filtering baru
    ...
    // --- Query Utama untuk Listing (Jika belum di-set oleh search) ---
    if (!isset($result)) {
        // Final execution jika query adalah string (bukan PDOStatement dari search)
        $stmt_final = $database->prepare($sqlmain);
        $stmt_final->execute($params);
        $result = $stmt_final;
    }


    // --- Query untuk Datalist (Harus dieksekusi di sini) ---
    // PERBAIKAN: Mengganti MySQLi query() dengan PDO query()
    $datalist_result = $database->query("SELECT pname, pemail FROM patient;");
      ...
      <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
        <tr>
          ...
          <td>
            ...
            <form action="" method="post" class="header-search">
              ...
              // Loop PDO fetch()
              while ($row00 = $datalist_result->fetch(PDO::FETCH_ASSOC)) {
                $d = $row00["pname"];
                $c = $row00["pemail"];
        ...
        <tr>
          <td colspan="4" style="padding-top:10px;">
            <p class="heading-main12" style="margin-left: 45px;font-size:18px;color:rgb(49, 49, 49)"><?php echo $selecttype . " Patients (" . $result->rowCount() . ")"; ?></p>
          </td>
