...
// PERBAIKAN: Tambahkan pengecekan session_status (jika tidak dijalankan di awal)
...
// PERBAIKAN: Konversi query() ke PDO
                    ...
                    <td>
                        ...
                        <form action="" method="post" class="header-search">
                            ...
                            <?php
                                ...
                                // PERBAIKAN: Konversi query() ke PDO untuk datalist
                                $list11 = $database->query("select docname,docemail from doctor;");

                                // PERBAIKAN: Mengganti num_rows dan fetch_assoc
...
?>
                ...
                <tr>
                    <td colspan="4" style="padding-top:10px;">
                        <p class="heading-main12" style="margin-left: 45px;font-size:18px;color:rgb(49, 49, 49)">All Doctors (<?php echo $list11->rowCount(); ?>)</p>
                    </td>
                ...
                <?php
                    if($_POST){
                        ...
                        $sqlmain= "select * from doctor where docemail ILIKE ? or docname ILIKE ? or docname ILIKE ? or docname ILIKE ? or docname ILIKE ?";
                        $stmt_search = $database->prepare($sqlmain);
                        $stmt_search->execute([$keyword, $keyword.'%', '%'.$keyword, '%'.$keyword.'%', $keyword]);
                        $result = $stmt_search;
                    }else{
                        // PERBAIKAN: Menggunakan PDO query()
                        ...
                        <tbody>
                            ...
                            <?php
                                ...
                                while ($row=$result->fetch(PDO::FETCH_ASSOC)){
                                    ...
                                    // PERBAIKAN: Menggunakan PDO Prepared Statement untuk specialties
                                    $sql_spe= "select sname from specialties where id=?";
                                    $stmt_spe = $database->prepare($sql_spe);
    ...
    if($_GET){
        ...
        if($action=='drop'){
            ...
            // PERBAIKAN: Konversi query view doctor ke PDO Prepared Statement
            ...
            // PERBAIKAN: Konversi query specialties ke PDO Prepared Statement
            $sql_spe= "select sname from specialties where id=?";
            $stmt_spe = $database->prepare($sql_spe);
            $stmt_spe->execute([$spe]);
            $spcil_array= $stmt_spe->fetch(PDO::FETCH_ASSOC);
            ...
            // PERBAIKAN: Konversi query edit doctor ke PDO Prepared Statement
            $sqlmain= "select * from doctor where docid=?";
            $stmt = $database->prepare($sqlmain);
            $stmt->execute([$id]);
            $row=$stmt->fetch(PDO::FETCH_ASSOC);
                                    ...
                                    <tr>
                                        <td class="label-td" colspan="2">
                                            <select name="spec" id="" class="box">';
                                                ...
                                                // PERBAIKAN: Konversi query specialties datalist di popup edit
