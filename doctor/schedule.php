...
$stmt_user = $database->prepare($sqlmain);
$stmt_user->execute([$useremail]);
$userfetch = $stmt_user->fetch(PDO::FETCH_ASSOC);
...
// PERBAIKAN: Mengganti query() MySQLi dengan PDO Prepared Statement dan menggunakan type casting
// Asumsi: schedule.docid adalah VARCHAR, doctor.docid adalah INTEGER. Kita cast $userid ke TEXT.
$sql_list = "SELECT * FROM schedule WHERE docid = ?::text";
 ...
 <div class="container">
            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <tr>
                    <td colspan="4" style="padding-top:10px;width: 100%;" >
                        ...
                        <p class="heading-main12" style="margin-left: 45px;font-size:18px;color:rgb(49, 49, 49)">My Sessions (<?php echo $list110->rowCount(); ?>) </p>
                    </td>
                ...
                $sqlmain_base = "SELECT schedule.scheduleid, schedule.title, doctor.docname, schedule.scheduledate, schedule.scheduletime, schedule.nop 
                                 FROM schedule 
                                 INNER JOIN doctor ON schedule.docid::text = doctor.docid::text 
                                 WHERE schedule.docid = ?::text "; // Filter utama: hanya sesi dokter ini
    ...
    if($_GET){
        ...
        if($action=='drop'){
            ...
            $sqlmain= "select schedule.scheduleid,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid::text=doctor.docid::text  where  schedule.scheduleid=?";
