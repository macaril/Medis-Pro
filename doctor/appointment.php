...
// PERBAIKAN: Konversi kueri pengambilan user dari MySQLi ke PDO Prepared Statement
...
<body>
  <div class="container">
    ...
    <div class="dash-body">
      <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <tr>
                  ...
                  <td width="30%">
                    <select name="docid" id="" class="box filter-container-items" style="width:90% ;height: 37px;margin: 0;">
                      ...
                      <?php
                      // PERBAIKAN: Mengganti query() MySQLi dengan PDO query() dan loop PDO fetch()
        ...
        <?php
        // Base Query
        // PERBAIKAN KRITIS: Mengganti JOIN schedule.docid=doctor.docid menjadi JOIN schedule.docid::int=doctor.docid
        $sqlmain_base = "SELECT appointment.appoid, schedule.scheduleid, schedule.title, doctor.docname, patient.pname, schedule.scheduledate, schedule.scheduletime, appointment.apponum, appointment.appodate
                         FROM schedule 
                         INNER JOIN appointment ON schedule.scheduleid = appointment.scheduleid 
                         INNER JOIN patient ON patient.pid = appointment.pid 
                         INNER JOIN doctor ON schedule.docid::int = doctor.docid 
                         WHERE doctor.docid = ?"; // Filter utama: hanya tampilkan appointment untuk dokter ini ($userid)
        
        $where_clauses = [];
        $params = [$userid]; // Parameter wajib untuk filter doctor.docid
        ...
        if ($_POST) {
          ...
          if (!empty($_POST["sheduledate"])) {
            ...
            $where_clauses[] = " schedule.scheduledate = ? ";
            $params[] = $sheduledate;
          }

          if (!empty($_POST["docid"])) {
            $docid = $_POST["docid"];
            // Jika memfilter berdasarkan docid lain, kita harus mengganti filter utamanya atau menambahkannya
            $where_clauses[] = " doctor.docid = ? ";
            $params[] = (int)$docid;
            
            // Note: Karena filter utama sudah ada (doctor.docid=$userid), logic ini akan menjadi AND
            // yang akan membuat hasil selalu kosong kecuali jika $docid == $userid.
            // Untuk file admin/appointment.php, filter docid adalah opsional, tetapi di file doctor/appointment.php, 
            // filter utama harus doctor.docid=$userid. Saya akan mempertahankan struktur asli.
          }
        }
        
        // Membangun Query
        $sqlmain = $sqlmain_base;
        if (count($where_clauses) > 0) {
            $sqlmain .= " AND " . implode(" AND ", $where_clauses);
        }
        
        $sqlmain .= " ORDER BY schedule.scheduledate DESC";
        
        // Final execution using PDO Prepared Statement (safe)
                    ...
                    if ($stmt_result->rowCount() == 0) {
                      ...
                      while ($row = $stmt_result->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo '<tr>
                          ...
                          <a href="delete-appointment.php?id=' . $appoid . '" class="non-style-link"><button  class="btn-primary-soft btn button-icon btn-delete"  style="padding-left: 40px;padding-top: 12px;padding-bottom: 12px;margin-top: 10px;"><font class="tn-in-text">Cancel</font></button></a>
                          &nbsp;&nbsp;&nbsp;</div>
                          </td>
                          </tr>';
  ...
  if ($_GET) {
    ...
    if ($action == 'add-session') {
      ...
      $sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.docname, schedule.scheduledate, schedule.scheduletime, schedule.nop FROM schedule INNER JOIN doctor ON schedule.docid::int=doctor.docid WHERE schedule.scheduleid=?";
          ...
          <div id="popup1" class="overlay">
            ...
            <center>
              ...
              <table width="80%" class="sub-table scrolldown add-doc-form-container" border="0">
                ...
                <tr>
                  <td class="label-td" colspan="2">
                    <label for="spec" class="form-label"><b>Patients that Already registerd for this session:</b> (' . $stmt12->rowCount() . "/" . $nop . ')</label>
                    <br><br>
                  </td>
                </tr>
                     ...
                     <tr> 
                       ...
                       if ($stmt12->rowCount() == 0) {
                          echo '<tr>
                              ...
                              <img src="../img/notfound.svg" width="25%">
                              <br>
                              <p class="heading-main12" style="margin-left: 45px;font-size:20px;color:rgb(49, 49, 49)">We  couldnt find any patients registered for this session !</p>
                              </center>
                              <br><br><br><br>
                              </td>
                          </tr>';
